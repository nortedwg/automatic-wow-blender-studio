...
class WoWFileData:
    ...
    def read_file(  self
        ...
        if WoWVersionManager().client_version < WoWVersions.WOD:
            ...
            if local_dir:
                if isinstance(identifier, int):
                    filepath = self.guess_filepath(identifier, file_format)
                    if filepath:
            ...
            if storage:
                ...
                if is_archive:
                    str_id = self.guess_filepath(identifier, file_format) if isinstance(identifier, int) else identifier
                    return storage.open(str_id.replace('/', '\\')).read(), ""
