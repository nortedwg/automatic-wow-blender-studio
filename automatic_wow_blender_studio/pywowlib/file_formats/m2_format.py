...
class M2String:
    ...
    def write(self, f):
        ...
        pos = f.tell()
        f.seek(ofs)
        f.write((self.value + '\0').encode('utf-8'))
        f.seek(pos)
...
class M2Texture:
    ...
    def write(self, f):
        ...
        uint32.write(f, self.flags)
        if self.type == 0 and int(getattr(self, 'fdid', 0) or 0) > 0:
            # Modern clients resolve hardcoded textures through TXID. Official
            # TXID-backed records leave the legacy filename array null; writing
            # a one-byte empty string here can make the client take the wrong
...
class M2Header:
    ...
    def write(self, f):
        header_size = self._size
        if self.m2_version >= M2Versions.WOTLK and self.global_flags & M2GlobalFlags.UseTextureCombinerCombos:
            header_size += 8

        MemoryManager.mem_reserve(f, header_size)
